...
import { percentV2MockData } from '../../lib/percentV2/mockData';

export function Header() {
  const walletPrefix = percentV2MockData.walletAddress.slice(0, 6);

  return (
    <div className="h-14 flex items-center justify-between px-8 bg-[#0a0a0a] border-b border-[#494949]">
      <div className="flex items-center gap-4 text-gray-400">
        <img
          src="/percent-v2/long-logo.svg"
          alt="percent.markets"
          className="h-8"
        />
        ...
        <span className="text-2xl" style={{ color: '#2D2D2D' }}>/</span>
        <div className="flex items-center gap-1.5">
          <img src="/percent-v2/solana-logo.jpg" alt="SOL" className="w-5 h-5 rounded-full" />
          <span className="text-white text-sm font-rinter font-semibold">
            {percentV2MockData.solBalance.toFixed(3)}
          </span>
        </div>
        <span className="text-2xl" style={{ color: '#2D2D2D' }}>/</span>
        <div className="flex items-center gap-1.5">
          <img src="/percent-v2/zc-logo.jpg" alt="ZC" className="w-5 h-5 rounded-full" />
          <span className="text-white text-sm font-rinter font-semibold">
            {percentV2MockData.zcBalance.toFixed(0)}
          </span>
        </div>
      </div>
